...
function Home() {
    ...
    const fetchQuizzes = useCallback(async () => {
        ...
        try {
            ...
            setPopup({ message: err.response?.data?.msg || "Error loading quizzes", type: "error", confirmAction: null, confirmInput: "" });
        } finally {
            setIsLoading(false);
        }
    }, [userEmail]);
    ...
    const filteredQuizzes = useMemo(() => {
        return quizzes.filter((q) => {
            ...
            const matchesDate =
                (!fromDate || (q.createdAt && new Date(q.createdAt).toLocaleDateString("en-US") >= new Date(fromDate).toLocaleDateString("en-US"))) &&
                (!toDate || (q.createdAt && new Date(q.createdAt).toLocaleDateString("en-US") <= new Date(toDate).toLocaleDateString("en-US")));
            return matchesAcademicYear && matchesPostedBy && matchesSubjectTitle && matchesYear && matchesBranch && matchesDivision && matchesDate;
        });
    }, [quizzes, filterAcademicYear, filterPostedBy, filterSubjectTitle, filterYear, filterBranch, filterDivision, fromDate, toDate]);
    ...
    const handleDelete = (quizId) => {
        setPopup({
            message: "Are you sure you want to delete this quiz? Type 'YES' to confirm.",
            type: "Confirm",
            confirmAction: async (input) => {
                if (input !== "YES") {
                    setPopup({ message: "Please type 'YES' to confirm deletion", type: "error", confirmAction: null, confirmInput: "" });
                    return;
                }
                setIsLoading(true);
                try {
                    ...
                    setPopup({ message: "Quiz deleted", type: "success", confirmAction: null, confirmInput: "" });
                    await fetchQuizzes();
                } catch (err) {
                    setPopup({ message: err.response?.data?.msg || "Error deleting quiz", type: "error", confirmAction: null, confirmInput: "" });
                } finally {
                    setIsLoading(false);
                }
            },
    ...
    const handleDuplicate = (quizId) => {
        setPopup({
            message: "Are you sure you want to duplicate this quiz? Type 'YES' to confirm.",
            type: "Confirm",
            confirmAction: async (input) => {
                if (input !== "YES") {
                    setPopup({ message: "Please type 'YES' to confirm duplication", type: "error", confirmAction: null, confirmInput: "" });
                    return;
                }
                setIsLoading(true);
                try {
                    ...
                    setPopup({ message: "Quiz duplicated successfully", type: "success", confirmAction: null, confirmInput: "" });
                    await fetchQuizzes();
                } catch (err) {
                    setPopup({ message: err.response?.data?.msg || "Failed to duplicate quiz", type: "error", confirmAction: null, confirmInput: "" });
                } finally {
                    setIsLoading(false);
                }
            },
    ...
    const handleToggleVisibility = async (quizId, isVisible) => {
        ...
        try {
            ...
            setPopup({ message: `Quiz is now ${isVisible ? "hidden" : "visible"}`, type: "success", confirmAction: null, confirmInput: "" });
            await fetchQuizzes();
        } catch (err) {
            setPopup({ message: err.response?.data?.msg || "Failed to toggle visibility", type: "error", confirmAction: null, confirmInput: "" });
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-4 sm:p-6">
            ...
            />
            {isLoading ? (
                <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-4 sm:p-6 flex items-center justify-center">
                ...
                <>
                    {role === "admin" && (
                        <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-4 sm:p-6 mb-4">
                            <button
                                onClick={toggleVisibleQuizzes}
                                className="w-full text-left text-lg font-bold text-gray-800 mb-3 flex items-center gap-2 focus:outline-none"
                                disabled={isLoading}
                            >
                            ...
                            {isVisibleQuizzesOpen && (
                                <div className="mt-2">
                                    {visibleQuizzes.length === 0 ? (
                                        <p className="text-xs md:text-sm text-gray-500">No quizzes are currently visible.</p>
                                    ) : (
                                        <ul className="space-y-2">
                                            {visibleQuizzes.map((q) => (
                                                <li
                                                    key={q._id}
                                                    className="p-3 bg-white rounded-md border border-gray-300 shadow-sm hover:shadow-md transition-shadow"
                                                >
                                                    <div className="flex flex-col sm:flex-row sm:justify-between gap-3">
                                                        <div className="flex-1">
                                                            <div>
                                                                <strong className="text-base font-bold text-gray-800 break-words">
                                                                    {q.subject} - {q.title}
                                                                </strong>
                                                                <p className="text-xs mt-0.5">
                                                                    <span className="text-green-600">
                                                                        {q.academicYear || "Not set"}
                                                                    </span>
                                                            ...
                                                            <div className="mt-2">
                                                                ...
                                                                <p className="text-xs md:text-sm text-gray-500">
                                                                    Year: {q.year || "All"} | Branch: {q.branch || "All"} | Division: {q.division || "All"}
                                                                </p>
                                                                <p className="text-xs md:text-sm text-gray-500">
                                                        ...
                                                        {role === "admin" && q.createdBy?.email === userEmail && (
                                                            <div className="flex sm:flex-col items-center justify-start sm:justify-center gap-1 mt-2 sm:mt-0">
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="text-yellow-500 hover:text-yellow-700 text-base p-1 rounded-full hover:bg-gray-100 transition"
                    ...
                    <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-4 sm:p-6">
                        <div className="flex items-center gap-2 mb-4">
                            <h1 className="text-lg font-bold text-gray-800 flex items-center gap-2">
                                <i className="fa-solid fa-list text-lg"></i> All Quizzes
                            </h1>
                        </div>
                        <div className="flex items-center gap-2 mb-2">
                            <label className="block font-semibold text-sm text-gray-700">Additional Filters</label>
                            <button
                                type="button"
                        ...
                        {isFilterOpen && (
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4 p-4">
                                ...
                                <div>
                                    <label className="block mb-1 font-semibold text-sm text-gray-700">Academic Year</label>
                                    <select
                                ...
                                <div>
                                    <label className="block mb-1 font-semibold text-sm text-gray-700">Posted by</label>
                                    <input
                                        type="text"
                        ...
                        {groupedQuizzes.sortedDates.length === 0 ? (
                            <p className="text-xs md:text-sm text-gray-500">No quizzes found.</p>
                        ) : (
