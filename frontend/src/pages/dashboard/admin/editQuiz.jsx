...
function EditQuiz() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (subject.length > 20) {
            setPopup({ message: "Subject must not exceed 20 characters", type: "error", confirmAction: null, confirmInput: "" });
            return;
        }
        if (title.trim() === "") {
        ...
        if (title.length > 40) {
            setPopup({ message: "Title must not exceed 40 characters", type: "error", confirmAction: null, confirmInput: "" });
            return;
        }
        ...
        try {
            ...
            await API.post(
                `/quiz/update/${quizId}`,
                { subject, title, questions: validQuestions, timer, academicYear, year, branch, division },
                { headers: { Authorization: `Bearer ${token}` } }
            );
            setQuestions(validQuestions);
