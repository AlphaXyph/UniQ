...
function Home() {
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-2 sm:p-4 md:p-6">
            <div className="max-w-full sm:max-w-4xl md:max-w-5xl mx-auto bg-white rounded-lg shadow-md p-3 sm:p-4 md:p-6">
                <Popup
                ...
                />
                <h1 className="text-base sm:text-lg md:text-xl font-bold mb-3 sm:mb-4 md:mb-6 flex items-center gap-2">
                    <i className="fa-solid fa-list text-base sm:text-lg"></i> All Quizzes
                </h1>
                {groupedQuizzes.sortedDates.length === 0 && (
                    <p className="text-xs sm:text-sm md:text-base text-gray-600">No quizzes found.</p>
                )}
                {groupedQuizzes.sortedDates.map((date) => (
                    groupedQuizzes.grouped[date] && (
                        <div key={date} className="mb-4 sm:mb-6 md:mb-8">
                            <h3 className="text-sm sm:text-base md:text-lg font-semibold text-gray-700 mb-2 sm:mb-3 md:mb-4">
                                {date}
                            </h3>
                            <ul className="space-y-3 sm:space-y-4">
                                {groupedQuizzes.grouped[date].map((q) => (
                                    <li
                                        key={q._id}
                                        className="p-3 sm:p-4 border border-gray-200 rounded-lg bg-white shadow-sm"
                                    >
                                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 sm:gap-4">
                                            <div className="flex-1">
                                                <strong className="text-sm sm:text-base md:text-lg font-medium break-words">
                                                    {q.subject} - {q.title}
                                                </strong>
                                                <p className="text-xs sm:text-sm text-blue-500 break-words">
                                                ...
                                                {role === "user" && q.isVisible ? (
                                                    <Link
                                                        ...
                                                        className="text-blue-600 hover:underline text-xs sm:text-sm inline-block mt-2 sm:mt-3"
                                                    >
                                                        Attempt Quiz
                                                    </Link>
                                                ) : role === "admin" ? (
                                            ...
                                            {role === "admin" && (
                                                <div className="flex gap-3 sm:gap-4 mt-3 sm:mt-0">
                                                    <button
                                                        ...
                                                        className={`text-${q.isVisible ? "gray" : "green"}-500 hover:text-${q.isVisible ? "gray" : "green"}-700 text-base sm:text-lg p-2`}
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-blue-500 hover:text-blue-700 text-base sm:text-lg p-2"
                                                        title="Edit Quiz"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="text-red-500 hover:text-red-700 text-base sm:text-lg p-2"
                                                        title="Delete Quiz"
                                                    >
